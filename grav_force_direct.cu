    ...
    __out devForce    iforce)
{
  ...
  {
    const float2 invq  = r2 > 0.0f ? (float2){rinv * pi.h, rinv * pj.h} : (float2){0.0f, 0.0f};
    const float2 q     = r2 > 0.0f ? (float2){1.0f/invq.x, 1.0f/invq.y} : (float2){0.0f, 0.0f};
